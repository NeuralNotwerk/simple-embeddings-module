...
class HierarchyConstrainedGrouping:
    ...
    def extract_chunks_with_metadata(self, file_path: str) -> List[ChunkMetadata]:
        ...
        logger.info(f"Extracting chunks with metadata from {file_path}")
        ...
        for i, chunk_text in enumerate(chunks):
            try:
                metadata = self._extract_chunk_metadata(
                    chunk_text, file_path, file_lines, i
                )
                if metadata:
                    chunk_metadata_list.append(metadata)
            except Exception as e:
                logger.error(f"Failed to extract metadata for chunk {i} in {file_path}: {e}")
                continue
        ...
        logger.info(f"Extracted {len(chunk_metadata_list)} chunks with metadata from {file_path}")
        return chunk_metadata_list
    
    def _extract_chunk_metadata(self, chunk_text: str, file_path: str, 
                               file_lines: List[str], chunk_index: int) -> Optional[ChunkMetadata]:
    ...
    def _analyze_chunk_hierarchy(self, chunk_text: str, file_path: str) -> Tuple[List[str], str]:
        ...
        for line in lines[:5]:  # Check first 5 lines
            ...
            if line.startswith('class '):
                ...
                func_name = self._extract_name_from_definition(line, 'def')
        ...
        try:
            ...
            for chunk in chunks:
                ...
                if len(text) > max_chars:
                    text = text[:max_chars] + "..."
                    logger.debug(f"Truncated chunk {chunk.id} from {len(chunk.text)} to {len(text)} chars")
                texts.append(text)
    ...
    def group_chunks_within_hierarchy(self, chunks: List[ChunkMetadata]) -> List[SemanticGroup]:
        ...
        logger.info(f"Grouping {len(chunks)} chunks within hierarchy constraints")
        ...
        for parent_scope, scope_chunks in scope_groups.items():
            if len(scope_chunks) < self.min_group_size:
                logger.debug(f"Skipping scope {parent_scope} with only {len(scope_chunks)} chunks")
                continue
            
            # Find semantic groups within this scope
            scope_semantic_groups = self._find_semantic_groups_in_scope(
                parent_scope, scope_chunks
            )
    ...
    def _find_semantic_groups_in_scope(self, parent_scope: str, 
                                     chunks: List[ChunkMetadata]) -> List[SemanticGroup]:
        ...
        if len(embeddings) < self.min_group_size:
            logger.debug(f"Not enough valid embeddings in scope {parent_scope}")
            return []
        ...
        for i, group_chunk_indices in enumerate(groups):
            if len(group_chunk_indices) >= self.min_group_size:
                ...
                semantic_group = self._create_semantic_group(
                    parent_scope, group_chunks, i
                )
    ...
    def _cluster_by_similarity(self, similarity_matrix: np.ndarray, 
                             chunks: List[ChunkMetadata]) -> List[List[int]]:
    ...
    def _create_semantic_group(self, parent_scope: str, chunks: List[ChunkMetadata], 
                             group_index: int) -> SemanticGroup:
    ...
    def _create_group_embedding(self, chunks: List[ChunkMetadata]) -> Optional[torch.Tensor]:
        ...
        try:
            ...
            if len(combined_text) > max_chars:
                ...
                logger.debug(f"Truncated group text to {len(combined_text)} chars")
            ...
            logger.error(f"Failed to create group embedding: {e}")
            return None
...
def process_file_with_hierarchy_grouping(file_path: str, 
                                       embedding_provider: EmbeddingProviderBase,
                                       similarity_threshold: float = 0.7) -> Tuple[List[ChunkMetadata], List[SemanticGroup]]:
...
def process_multiple_files_with_grouping(file_paths: List[str],
                                       embedding_provider: EmbeddingProviderBase,
                                       similarity_threshold: float = 0.7) -> Tuple[List[ChunkMetadata], List[SemanticGroup]]:
    ...
    for file_path in file_paths:
        try:
            chunks, groups = process_file_with_hierarchy_grouping(
                file_path, embedding_provider, similarity_threshold
            )
