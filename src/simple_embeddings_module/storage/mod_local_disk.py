...
class LocalDiskStorage(StorageBackendBase):
    ...
    def __init__(self, **config):
        ...
        logger.info(f"LocalDiskStorage initialized: {self.base_path}")
    ...
    def _init_serializer(self):
        ...
        try:
            ...
            raise StorageBackendError(
                "orjson library not installed. Install with: pip install orjson"
            )

    def save_index(
        self, vectors: torch.Tensor, metadata: Dict[str, Any], index_name: str
    ) -> bool:
        ...
        try:
            ...
            raise StorageBackendError(f"Failed to save index: {e}")

    def load_index(
        self, index_name: str, device: Optional[torch.device] = None
    ) -> Tuple[torch.Tensor, Dict[str, Any]]:
        """Load vector index and metadata from local disk"""
        self.validate_index_name(index_name)

        if not self.index_exists(index_name):
            raise StorageBackendError(f"Index '{index_name}' does not exist")
        ...
        try:
            ...
            if not index_file.exists():
                raise StorageBackendError(f"Index file not found for '{index_name}'")
            ...
            raise StorageBackendError(f"Failed to load index: {e}")
    ...
    def delete_index(self, index_name: str) -> bool:
        ...
        try:
            ...
            raise StorageBackendError(f"Failed to delete index: {e}")
    ...
    def index_exists(self, index_name: str) -> bool:
        ...
        try:
            ...
            return index_dir.is_dir() and (
                ...
                or (index_dir / "index.json.gz").exists()
            )
        except Exception:
            return False
    ...
    def _write_metadata_summary(
        self, index_dir: Path, metadata: Dict[str, Any]
    ) -> None:
        """Write metadata summary for quick access"""
        try:
            summary = {
            ...
            logger.warning(f"Failed to write metadata summary: {e}")
