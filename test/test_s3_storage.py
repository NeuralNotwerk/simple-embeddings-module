...
try:
    ...
    from src.simple_embeddings_module.storage.mod_storage_base import (
        StorageBackendError,
    )
    # Check if boto3 is available
    BOTO3_AVAILABLE = importlib.util.find_spec("boto3") is not None
    DEPENDENCIES_AVAILABLE = BOTO3_AVAILABLE
...
def test_s3_storage_basic():
    ...
    try:
        ...
        if not exists:
            print("❌ Index should exist but doesn't")
            return False
        # Test 4: List indexes
        ...
        if index_name not in indexes:
            print("❌ Test index not found in list")
            return False
        show_raw_s3_operations(
            "List Indexes",
            "list_indexes",
        ...
        if exists_after_delete:
            print("❌ Index still exists after deletion")
            return False
        print("✅ Index deletion verified")
        show_raw_s3_operations(
            "Delete Index",
            "delete_index",
            {
...
def test_s3_error_handling():
    ...
    if bucket_name:
        ...
        try:
            ...
            print("❌ Should have failed loading non-existent index")
            return False
        except StorageBackendError as e:
...
def main():
    ...
    parser.add_argument("--errors-only", action="store_true", help="Run only error handling tests")
