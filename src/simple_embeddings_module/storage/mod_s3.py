...
class S3Storage(StorageBackendBase):
    ...
    def _verify_bucket_access(self) -> None:
        ...
        try:
            ...
            logger.info("Successfully verified access to S3 bucket '%s'", self.bucket_name)
        except ClientError as e:
            error_code = e.response["Error"]["Code"]
            if error_code == "NoSuchBucket":
                raise StorageBackendError("S3 bucket '%s' does not exist" % self.bucket_name)
            elif error_code == "AccessDenied":
                raise StorageBackendError("Access denied to S3 bucket '%s'" % self.bucket_name)
            else:
                raise StorageBackendError("Failed to access S3 bucket '%s': %s" % (self.bucket_name, e))
    ...
    def save_index(self, vectors: torch.Tensor, metadata: Dict[str, Any], index_name: str) -> bool:
        ...
        try:
            ...
            self._upload_object(metadata_key, metadata_bytes, {"index_name": index_name, "file_type": "metadata"})
            # Upload vectors
            self._upload_object(vectors_key, vectors_bytes, {"index_name": index_name, "file_type": "vectors"})
            logger.info("Successfully saved index '%s' to S3", index_name)
            return True
        except Exception as e:
            logger.error("Failed to save index '%s' to S3: %s", index_name, e)
    ...
    def load_index(self, index_name: str, device: Optional[torch.device] = None) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        try:
            ...
            logger.info("Successfully loaded index '%s' from S3", index_name)
    ...
    def delete_index(self, index_name: str) -> bool:
        ...
        try:
            ...
            if not objects_to_delete:
                logger.warning("No objects found for index '%s'", index_name)
                return False
            ...
            for i in range(0, len(objects_to_delete), batch_size):
                ...
                if "Errors" in response and response["Errors"]:
                    ...
                    logger.error("Failed to delete some objects for index '%s': %s", index_name, errors)
                    return False
    ...
    def get_index_info(self, index_name: str) -> Optional[IndexInfo]:
        ...
        try:
            ...
            try:
                ...
                logger.warning("Could not load metadata for detailed info: %s", e)
                document_count = 0
                embedding_dim = 0
                model_name = None
            return IndexInfo(
