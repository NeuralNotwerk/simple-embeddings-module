...
SEMANTIC_BOUNDARY_NODES = {
    "python": {"function_definition", "class_definition", "async_function_definition", "decorated_definition"},
    "javascript": {
...
def ts_get_code_chunks(file_name: str) -> List[str]:
    ...
    try:
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            code = f.read()
        if not code.strip():
            logger.warning("Empty file: %s", file_name)
            return []
        ...
        if not line_breaks:
            ...
            logger.debug("No semantic boundaries found in %s, returning as single chunk", file_name)
            return [code]
        ...
        logger.error("Error chunking file %s: %s", file_name, e)
...
def _get_code_line_breaks(code: str, file_name: str = "") -> List[int]:
    ...
    try:
        ...
        if language is None:
            logger.debug("No tree-sitter parser available for %s", file_name)
            return []
        ...
        logger.debug("Found %s semantic boundaries in %s", len(boundaries), file_name)
        return line_breaks
    except Exception as e:
        logger.error("Error parsing code with tree-sitter: %s", e)
        return []
def _find_semantic_boundaries(
    node: tree_sitter.Node, boundary_types: Set[str], boundaries: List[tree_sitter.Node]
) -> None:
    """
