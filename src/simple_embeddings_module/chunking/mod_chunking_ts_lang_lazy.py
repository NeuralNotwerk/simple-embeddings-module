...
class TreeSitterLanguageLoader:
    ...
    def get_language(self, package_name: str) -> Optional[Any]:
        ...
        try:
            ...
            if language_capsule is None:
                logger.warning(f"Could not find language function in {package_name}")
            ...
            logger.debug(f"Loaded tree-sitter language: {package_name}")
            return language
            
        except ImportError as e:
            logger.debug(f"Failed to import {package_name}: {e}")
            self._failed_imports.add(package_name)
            return None
        except Exception as e:
            logger.warning(f"Error loading {package_name}: {e}")
            self._failed_imports.add(package_name)
            return None
