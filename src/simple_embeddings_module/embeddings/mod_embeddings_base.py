...
class EmbeddingProviderBase(ABC):
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        ...
        base_capabilities = {
            ...
            "device_preferences": getattr(self, "device_preferences", ["cuda", "mps", "cpu"]),
    ...
    def validate_inputs(self, texts: List[str]) -> None:
        ...
        for i, text in enumerate(texts):
            if not isinstance(text, str):
                raise ValueError("Input %s must be a string, got %s" % (i, type(text)))
            if len(text) > max_length:
                raise ValueError("Input %s exceeds max length %s: %s characters" % (i, max_length, len(text)))
