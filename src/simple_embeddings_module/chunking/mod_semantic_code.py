...
class SemanticCodeClusteringProvider(ChunkingProviderBase):
    ...
    CONFIG_PARAMETERS = {
        ...
        "min_cluster_size": {"type": int, "default": 1, "description": "Minimum number of functions per cluster"},
        "max_cluster_size": {"type": int, "default": 8, "description": "Maximum number of functions per cluster"},
        "include_context": {
            "type": bool,
            "default": True,
    ...
    def initialize(self, config: Dict[str, Any], embedding_provider) -> None:
        ...
        logger.info("Initialized semantic code clustering with threshold=%s", self.similarity_threshold)
    def chunk_text(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info("Created %s semantic code chunks from %s units", len(all_chunks), len(semantic_units))
            return all_chunks
        except Exception as e:
            logger.error("Error in semantic code clustering: %s", e)
    ...
    def _extract_semantic_units(
        self, root_node: tree_sitter.Node, text: str, language: tree_sitter.Language
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_unit_name(self, node: tree_sitter.Node, text: str) -> Optional[str]:
        ...
        try:
            ...
            logger.warning("Failed to extract unit name: %s", e)
            return None
    def _group_by_scope(self, units: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        return scoped_units
    def _cluster_units_in_scope(
        self, units: List[Dict[str, Any]], scope_name: str, metadata: Optional[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        if len(units) <= 1:
            ...
            for unit in units:
                ...
                chunks.append({"text": unit["text"], "metadata": chunk_metadata})
            return chunks
        try:
            # Embed each unit
            embeddings = []
            ...
            logger.error("Error clustering units in scope %s: %s", scope_name, e)
    ...
    def _units_as_individual_chunks(
        self, units: List[Dict[str, Any]], scope_name: str, metadata: Optional[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
