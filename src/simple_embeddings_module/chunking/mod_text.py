...
class TextChunkingProvider(ChunkingProviderBase):
    ...
    def __init__(self, **config):
        ...
        if embedding_provider is None:
            raise ChunkingProviderError(
                "embedding_provider is required for chunking initialization"
            )
    ...
    def _configure_for_embedding_provider(self) -> None:
        ...
        logger.info(
            f"Configured for embedding provider: chunk_size={self.chunk_size}, "
            f"overlap={self.overlap_size}, min_size={self.min_chunk_size}"
        )
