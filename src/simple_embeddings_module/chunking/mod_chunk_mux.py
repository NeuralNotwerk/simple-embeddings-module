...
class ChunkMultiplexer(ChunkingProviderBase):
    ...
    def _compile_routing_rules(self) -> None:
        ...
        for rule in self.routing_rules:
            if not isinstance(rule, dict):
                raise ChunkingProviderError(f"Invalid routing rule format: {rule}")
            ...
            if not pattern or not strategy:
                raise ChunkingProviderError(
                    f"Routing rule missing pattern or strategy: {rule}"
                )
            ...
            try:
                ...
                raise ChunkingProviderError(f"Invalid regex pattern '{pattern}': {e}")

    def register_chunking_strategy(
        self, name: str, strategy: ChunkingProviderBase
    ) -> None:
    ...
    def chunk_document(
        ...
        if strategy_name not in self._chunking_strategies:
            raise ChunkingProviderError(
                f"Chunking strategy '{strategy_name}' not registered"
            )
