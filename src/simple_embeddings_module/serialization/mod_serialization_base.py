...
class SerializationProviderBase(ABC):
    ...
    def validate_tensor(self, tensor: torch.Tensor) -> None:
        ...
        if not isinstance(tensor, torch.Tensor):
            raise ValueError("Expected torch.Tensor, got %s" % type(tensor))
        if tensor.numel() == 0:
        ...
        if max_size > 0:
            ...
            if tensor_bytes > max_size:
                raise ValueError("Tensor size %s exceeds maximum %s" % (tensor_bytes, max_size))
    def validate_metadata(self, metadata: Dict[str, Any]) -> None:
        """Validate metadata before serialization"""
        if not isinstance(metadata, dict):
            raise ValueError("Expected dict, got %s" % type(metadata))
        ...
        if isinstance(obj, allowed_types):
            ...
            for key, value in obj.items():
                if not isinstance(key, str):
                    raise ValueError("Non-string key at %s.%s" % (path, key))
                self._validate_json_serializable(value, "%s.%s" % (path, key))
        elif isinstance(obj, list):
            for i, item in enumerate(obj):
                self._validate_json_serializable(item, "%s[%s]" % (path, i))
        else:
            raise ValueError("Non-JSON-serializable type %s at %s" % (type(obj), path))
