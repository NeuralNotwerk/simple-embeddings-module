...
class SemanticSimilarityChunkingProvider(ChunkingProviderBase):
    ...
    CONFIG_PARAMETERS = {
        ...
        'min_cluster_size': {
            'type': int,
            'default': 2,
            'description': 'Minimum number of units per cluster'
        },
        'max_cluster_size': {
            'type': int,
            'default': 10,
            'description': 'Maximum number of units per cluster'
        },
        'unit_type': {
            'type': str,
            'default': 'sentence',
    ...
    def initialize(self, config: Dict[str, Any], embedding_provider) -> None:
        ...
        logger.info(f"Initialized semantic similarity chunking with threshold={self.similarity_threshold}")
    ...
    def chunk_text(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Created {len(chunks)} semantic chunks from {len(units)} units")
            return chunks

        except Exception as e:
            logger.error(f"Error in semantic similarity chunking: {e}")
    ...
    def _embed_units(self, units: List[str]) -> np.ndarray:
        ...
        for unit in units:
            try:
                ...
                logger.warning(f"Failed to embed unit: {e}")
    ...
    def _hierarchical_clustering(self, units: List[str], embeddings: np.ndarray) -> List[List[int]]:
        ...
        try:
            ...
            logger.warning(f"Hierarchical clustering failed: {e}, falling back to threshold")
            return self._threshold_clustering(units, embeddings)

    def _create_chunks_from_clusters(self, clusters: List[List[int]], metadata: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
