...
def test_imports():
    ...
    try:
        ...
        from simple_embeddings_module.serialization import SerializationProviderBase
        from simple_embeddings_module.storage import StorageBackendBase
...
def test_end_to_end():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        success, stdout, stderr = run_command(
            "sem-cli init --name smoke_test --path ./test_indexes"
        )
        if not success:
        ...
        success, stdout, stderr = run_command(
            "sem-cli add --files doc1.txt doc2.txt doc3.txt --path ./test_indexes"
        )
        if not success:
        ...
        if (
            ...
            and "machine learning" in stdout.lower()
        ):
