...
class HierarchyGroupingProvider(ChunkingProviderBase):
    ...
    def chunk_text(self, text: str, file_path: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Failed to chunk text with hierarchy grouping: {e}")
            return [text]
    ...
    def chunk_file(self, file_path: str) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Failed to chunk file with hierarchy grouping: {e}")
            return []
    ...
    def chunk_multiple_files_with_metadata(self, file_paths: List[str]) -> Tuple[List[ChunkMetadata], List[SemanticGroup]]:
...
class HierarchyGroupingStorage:
    ...
    @staticmethod
    def serialize_chunks_and_groups(chunks: List[ChunkMetadata], 
                                   groups: List[SemanticGroup]) -> Dict[str, Any]:
    ...
    def deserialize_chunks_and_groups(data: Dict[str, Any]) -> Tuple[List[ChunkMetadata], List[SemanticGroup]]:
        ...
        for group_data in data.get("semantic_groups", []):
            ...
            creation_timestamp = datetime.fromisoformat(group_data.get("creation_timestamp", datetime.now().isoformat()))
    ...
    @staticmethod
    def save_to_file(chunks: List[ChunkMetadata], groups: List[SemanticGroup], 
                    file_path: str) -> None:
        ...
        logger.info(f"Saved {len(chunks)} chunks and {len(groups)} groups to {file_path}")
    ...
    def load_from_file(file_path: str) -> Tuple[List[ChunkMetadata], List[SemanticGroup]]:
        ...
        logger.info(f"Loaded {len(chunks)} chunks and {len(groups)} groups from {file_path}")
...
class HierarchyGroupingSearch:
    ...
    def search_chunks_and_groups(self, query: str, 
    ...
    def search_within_scope(self, query: str, scope: str,
                           chunks: List[ChunkMetadata],
                           groups: List[SemanticGroup],
...
def create_hierarchy_grouping_demo(file_paths: List[str], 
                                 embedding_provider: EmbeddingProviderBase,
                                 output_file: Optional[str] = None) -> Tuple[List[ChunkMetadata], List[SemanticGroup]]:
    """Create a demo of hierarchy-constrained grouping with multiple files."""
    logger.info(f"Creating hierarchy grouping demo with {len(file_paths)} files")
    
    # Process all files
    chunks, groups = process_multiple_files_with_grouping(
        file_paths, embedding_provider, similarity_threshold=0.6
    )
    ...
    # Print summary
    print(f"🎯 Hierarchy-Constrained Grouping Demo Results:")
    ...
    for file_path, stats in file_stats.items():
        ...
        print(f"   {file_name}: {stats['chunks']} chunks, {stats['groups']} groups")
