...
class CodeChunkingProvider(ChunkingProviderBase):
    ...
    def chunk_file(self, file_path: str) -> List[str]:
        ...
        try:
            ...
            logger.debug("Generated %s semantic chunks for %s", len(processed_chunks), file_path)
            ...
            if self.config["fallback_to_text"]:
                try:
                    ...
                    logger.error("Fallback chunking also failed: %s", fallback_error)
                    return []
            else:
                raise
    ...
    def _extract_imports(self, first_chunk: str) -> str:
        ...
        for line in lines:
            stripped = line.strip()
            if (stripped.startswith(("import ", "from ", "#include", "using ", "use ")) or stripped.startswith("package ") or stripped.startswith("namespace ")):
    ...
    def _configure_for_embedding_provider(self) -> None:
        ...
        if hasattr(self, "embedding_provider") and self.embedding_provider:
            ...
            if estimated_max_chars < self.config["max_chunk_size"]:
                ...
                logger.info("Adjusted max_chunk_size to %s based on embedding model", estimated_max_chars)
    ...
    def chunk_document(
        ...
        for i, chunk_text in enumerate(chunks):
            ...
            if metadata:
                chunk_metadata.update({k: v for k, v in metadata.items() if k not in chunk_metadata})
            chunk_info.append({"text": chunk_text, "metadata": chunk_metadata})
        return chunk_info
