...
class ChunkingProviderBase(ABC):
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        ...
        base_capabilities = {
            ...
            "boundary_type": getattr(self, "boundary_type", ChunkBoundary.SENTENCE.value),
    ...
    def is_compatible_with_embedding_provider(self, other_provider: EmbeddingProviderBase) -> Tuple[bool, List[str]]:
        ...
        reasons.append("⚠️  BEST EFFORT: 'Compatible' providers may still produce different results due to:")
        ...
        if other_capabilities["embedding_dim"] != current_capabilities["embedding_dim"]:
            reasons.append(
                "Embedding dimensions differ: %s vs %s" % (current_capabilities['embedding_dim'], other_capabilities['embedding_dim'])
            )
        ...
        if length_ratio > 0.2:
            reasons.append(
                "Max sequence length differs significantly: %s vs %s (%.1f%% difference)" % (current_max_length, other_max_length, length_ratio * 100)
            )
        ...
        if not self._are_models_compatible(current_model, other_model):
            reasons.append(
                "Models are not compatible: '%s' vs '%s'" % (current_capabilities.get('model_name'), other_capabilities.get('model_name'))
            )
        ...
        if current_chunk_capabilities["max_chunk_size"] > other_max_length:
            reasons.append(
                "Current chunks too large for new provider: %s > %s" % (current_chunk_capabilities['max_chunk_size'], other_max_length)
            )
        ...
        if current_tokenizer and other_tokenizer and current_tokenizer != other_tokenizer:
            reasons.append("Tokenizer types differ: %s vs %s" % (current_tokenizer, other_tokenizer))
    ...
    def _are_quantization_variants(self, model1: str, model2: str) -> bool:
        ...
        for indicator in quant_indicators:
            clean1 = clean1.replace(f"-{indicator}", "").replace(f"_{indicator}", "").replace(indicator, "")
            clean2 = clean2.replace(f"-{indicator}", "").replace(f"_{indicator}", "").replace(indicator, "")
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f"max_chunk={caps.get('max_chunk_size', 'unknown')}, "
            f"boundary={caps.get('boundary_type', 'unknown')}, "
