...
def show_raw_output(title: str, chunks: List[ChunkMetadata], groups: List[SemanticGroup]):
    ...
    for i, group in enumerate(groups, 1):
        ...
        print(
            f"  Group Embedding shape: {group.group_embedding.shape if group.group_embedding is not None else 'None'}"
        )
...
def test_hierarchy_grouping():
    ...
    test_files = {"sample_python.py": SAMPLE_PYTHON_CODE, "sample_javascript.js": SAMPLE_JAVASCRIPT_CODE}
    ...
    try:
        ...
        if VERBOSE:
            ...
            for chunk in all_chunks:
                scope = (
                    "::".join(chunk.parent_hierarchy[:-1])
                    if len(chunk.parent_hierarchy) > 1
                    else chunk.parent_hierarchy[0]
                )
        ...
        for chunk in all_chunks:
            scope = (
                "::".join(chunk.parent_hierarchy[:-1]) if len(chunk.parent_hierarchy) > 1 else chunk.parent_hierarchy[0]
            )
        ...
        if VERBOSE:
            ...
            for scope, count in scope_distribution.items():
                ...
                scope_chunks = [
                    c
                    for c in all_chunks
                    if ("::".join(c.parent_hierarchy[:-1]) if len(c.parent_hierarchy) > 1 else c.parent_hierarchy[0])
                    == scope
                ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Enable verbose output showing raw inputs and outputs"
    )
