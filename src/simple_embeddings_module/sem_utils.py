...
def load_config(json_file: str) -> SEMConfigObject:
    ...
    if not config_path.exists():
        raise FileNotFoundError("Configuration file not found: %s" % json_file)
    try:
        with open(config_path, "rb") as f:
            config_data = orjson.loads(f.read())
        logger.info("Loaded configuration from: %s", json_file)
        return SEMConfigObject(config_data)
    except orjson.JSONDecodeError as e:
        raise ValueError("Invalid JSON in configuration file '%s': %s" % (json_file, e))
    except Exception as e:
        raise ValueError("Failed to load configuration from '%s': %s" % (json_file, e))
...
def save_config(config: Union[SEMConfigObject, Dict[str, Any]], json_file: str) -> None:
    ...
    try:
        ...
        json_bytes = orjson.dumps(config_data, option=orjson.OPT_INDENT_2 | orjson.OPT_SORT_KEYS)
        with open(config_path, "wb") as f:
            f.write(json_bytes)
        logger.info("Saved configuration to: %s", json_file)
    except Exception as e:
        raise ValueError("Failed to save configuration to '%s': %s" % (json_file, e))

def generate_config_template(json_file: str = None, **kwargs) -> Union[bool, SEMConfigObject]:
    ...
    * Source of truth for config options
    ...
    if json_file:
        ...
        try:
            ...
            logger.info("Generated configuration template: %s", json_file)
            return True
        except Exception as e:
            logger.error("Failed to generate template '%s': %s", json_file, e)
            return False
    else:
        # Return as object
...
def validate_config(
    ...
    for section in required_sections:
        ...
        if not isinstance(section_config, dict):
            errors.append("Section '%s' must be a dictionary" % section)
            continue
        ...
        if section == "embedding":
            ...
            if section_config.get("backend") == "local_disk" and "path" not in section_config:
...
def get_config_info(config: Union[SEMConfigObject, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if isinstance(config, SEMConfigObject):
        ...
        config_dict = config
    info = {
        "embedding_provider": config_dict.get("embedding", {}).get("provider", "unknown"),
        ...
        "serialization_provider": config_dict.get("serialization", {}).get("provider", "unknown"),
