...
class CompatibilityResult:
    ...
    def __post_init__(self):
        ...
        if self.is_compatible:
            ...
            self.reasons.insert(1, "   • Different hardware (NVIDIA vs AMD vs Apple Silicon)")
            ...
            self.reasons.insert(5, "   • Always validate results after switching providers!")
...
class SEMConfigBuilder:
    ...
    def set_embedding_provider(self, provider_name: str, **provider_config) -> "SEMConfigBuilder":
        ...
        self._embedding_provider = self._registry.instantiate_module("embeddings", provider_name, provider_config)
        # Store configuration
        self.config["embedding"] = {"provider": provider_name, **provider_config}
        logger.info("Set embedding provider: %s", provider_name)
        return self

    def set_chunking_strategy(self, strategy_name: str, **chunking_config) -> "SEMConfigBuilder":
        ...
        logger.info(
            "Instantiating chunking provider '%s' with config keys: %s", strategy_name, list(chunking_config_with_embedding.keys())
        )
        ...
        logger.info("Set chunking strategy: %s", strategy_name)
        return self

    def auto_configure_chunking(self, strategy_preference: Optional[str] = None) -> "SEMConfigBuilder":
        ...
        """
        if self._embedding_provider is None:
            raise ValueError("Must set embedding provider before auto-configuring chunking")
    ...
    def _select_optimal_chunking_strategy(self, embedding_caps: Dict[str, Any]) -> str:
        ...
        if "code" in model_name and max_length > 2048:
            return "text"  # Use text chunking for now (code chunking not implemented yet)
        elif max_length < 256:
            return "text"  # Simple text chunking for short context models
        else:
            return "text"  # Use text chunking as default for MVP

    def _generate_chunking_config(self, embedding_caps: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def set_storage_backend(self, backend_name: str, **storage_config) -> "SEMConfigBuilder":
        ...
        logger.info("Set storage backend: %s", backend_name)
        return self

    def set_serialization_provider(self, provider_name: str, **serialization_config) -> "SEMConfigBuilder":
        ...
        logger.info("Set serialization provider: %s", provider_name)
        return self
    ...
    def set_index_config(self, index_name: str, **index_config) -> "SEMConfigBuilder":
        ...
        logger.info("Set index config: %s", index_name)
        return self
    ...
    def build(self) -> Dict[str, Any]:
        ...
        if not self.config["index"]:
            self.set_index_config("sem_simple_index", max_documents=100000, similarity_threshold=0.1)
    ...
    def check_provider_compatibility(self, new_provider_name: str, **new_provider_config) -> CompatibilityResult:
        ...
        """
        if self._embedding_provider is None:
            return CompatibilityResult(is_compatible=True, reasons=[], migration_required=False)
        # Instantiate new provider
        try:
            new_provider = self._registry.instantiate_module("embedding", new_provider_name, new_provider_config)
        except Exception as e:
            return CompatibilityResult(
                is_compatible=False,
                reasons=["Failed to instantiate new provider: %s" % e],
        ...
        if current_caps["embedding_dim"] != new_caps["embedding_dim"]:
            reasons.append(
                "❌ Embedding dimensions differ: %s vs %s (INCOMPATIBLE)" % (current_caps['embedding_dim'], new_caps['embedding_dim'])
            )
        ...
        if not self._are_models_compatible(current_model, new_model):
            reasons.append("❌ Models are not compatible: '%s' vs '%s' (INCOMPATIBLE)" % (current_model, new_model))
        ...
        if length_ratio > 0.2:
            reasons.append(
                "❌ Max sequence length differs significantly: %s vs %s (%.1f%% difference)" % (current_max_length, new_max_length, length_ratio * 100)
            )
        elif length_ratio > 0.05:
            reasons.append(
                "⚠️  Max sequence length differs moderately: %s vs %s (%.1f%% difference)" % (current_max_length, new_max_length, length_ratio * 100)
            )
        # 4. Check chunking compatibility if chunking provider exists
        if self._chunking_provider:
            chunk_compatible, chunk_reasons = self._chunking_provider.is_compatible_with_embedding_provider(
                new_provider
            )
            if not chunk_compatible:
                reasons.extend(["❌ Chunking: %s" % reason for reason in chunk_reasons])
        ...
        if current_tokenizer and new_tokenizer and current_tokenizer != new_tokenizer:
            reasons.append("⚠️  Tokenizer types differ: %s vs %s" % (current_tokenizer, new_tokenizer))
        ...
        if current_norm != new_norm:
            reasons.append("❌ Normalization differs: %s vs %s (INCOMPATIBLE)" % (current_norm, new_norm))
    ...
    def create_migration_config(self, new_provider_name: str, **new_provider_config) -> Dict[str, Any]:
...
def create_code_search_config() -> Dict[str, Any]:
    ...
    builder = SEMConfigBuilder()
    builder.set_embedding_provider("sentence_transformers", model="Salesforce/SFR-Embedding-Code-400M_R")
...
def create_research_config() -> Dict[str, Any]:
    ...
    builder.set_index_config("research_papers", max_documents=100000, similarity_threshold=0.75)
    return builder.build()
